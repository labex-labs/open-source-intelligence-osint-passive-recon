# 开源情报 (OSINT) 与被动侦察

**支持语言:** [English](README.md) · [简体中文](README_zh.md) · [Español](README_es.md) · [Français](README_fr.md) · [Deutsch](README_de.md) · [日本語](README_ja.md) · [Русский](README_ru.md) · [한국어](README_ko.md) · [Português](README_pt.md)

<p align="center">
  <a href="https://labex.io/zh/courses/open-source-intelligence-osint-passive-recon">
    <img src="https://course-cover.labex.io/open-source-intelligence-osint-passive-recon.png?lang=zh" alt="开源情报 (OSINT) 与被动侦察">
  </a>
</p>

利用开源情报 (OSINT) 技术进行被动侦察。掌握域名枚举、DNS 情报收集以及公共数据采集，从而绘制出组织外部的攻击面。

[在 LabEx 开始课程](https://labex.io/zh/courses/open-source-intelligence-osint-passive-recon)

## 练习

|   序号 | 名称           | 难度   | 练习                                                                                                                                                          |
|------|--------------|------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|
|    1 | 域名与 Whois 枚举 | 初级   | <a target='_blank' href='https://labex.io/zh/labs/linux-domain-and-whois-enumeration-656132?course=open-source-intelligence-osint-passive-recon'>开始实验</a>   |
|    2 | DNS 信息收集     | 初级   | <a target='_blank' href='https://labex.io/zh/labs/linux-dns-intelligence-gathering-656131?course=open-source-intelligence-osint-passive-recon'>开始实验</a>     |
|    3 | 收集公开信息       | 初级   | <a target='_blank' href='https://labex.io/zh/labs/linux-harvesting-public-information-656133?course=open-source-intelligence-osint-passive-recon'>开始实验</a>  |
|    4 | 被动侦察任务       | 中级   | <a target='_blank' href='https://labex.io/zh/labs/linux-passive-reconnaissance-mission-656130?course=open-source-intelligence-osint-passive-recon'>开始挑战</a> |

## About LabEx

<div align="left"><p><a href="https://labex.io"><strong>LabEx</strong></a> is a <strong>hands-on learning platform for beginners</strong>.</p><p>Explore <a href="https://labex.io/learn/linux"><strong>Linux</strong></a>, <a href="https://labex.io/learn/devops"><strong>DevOps</strong></a>, <a href="https://labex.io/learn/cybersecurity"><strong>Cybersecurity</strong></a>, and <strong>more</strong> — all directly in your browser.</p><p>Learn step by step through <strong>interactive labs</strong>, <strong>guided exercises</strong>, and <strong>real-world projects</strong>. 🌱<br />No setup, no stress — just practice and grow your skills by doing.</p><br /><p><a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/assets/elements/badges/download-on-the-app-store.svg" alt="Download on the App Store" height="54" /></a>&nbsp;<a href="https://apps.apple.com/app/id6765840991"><img src="https://developer.apple.com/app-store/marketing/guidelines/images/badge-download-on-the-mac-app-store.svg" alt="Download on the Mac App Store" height="52" /></a></p><br /><p>📖 Need help? Visit our <a href="https://support.labex.io/">Help Center</a> or email info@labex.io</p></div>

